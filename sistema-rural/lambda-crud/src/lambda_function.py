...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if method == "POST" and "/properties" in resource and "{id}" not in resource:
            ...
            return create_response(404, {"error": f"Endpoint não encontrado: {method} {resource}"})
...
def get_properties(event: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        properties = [format_property_for_response(item) for item in response.get("Items", [])]
...
def update_property(event: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if "coordinates" in body:
            ...
            expression_values[":coordinates"] = convert_coordinates_to_decimal(body["coordinates"])
...
def extract_user_id(event: Dict[str, Any]) -> str:
    ...
    try:
        ...
        return (
            authorizer_context.get("claims", {}).get("sub") or
            authorizer_context.get("sub")
        )
    except:
        return None
...
def convert_coordinates_to_decimal(coordinates):
    ...
    if not isinstance(coordinates, list):
        return coordinates
    return [[Decimal(str(coord[0])), Decimal(str(coord[1]))] for coord in coordinates if len(coord) == 2]
...
def format_property_for_response(property_item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "coordinates": [[float(coord[0]), float(coord[1])] for coord in property_item.get("coordinates", [])],
