...
def handle_direct_event(event_data):
    ...
    if event_type == "analysis_completed":
        send_analysis_notification(
            event_data["propertyId"], event_data["userId"], event_data["analysisData"]
        )
    elif event_type == "property_notification":
        send_property_notification(
            event_data["userId"], event_data["topic"], event_data["message"]
        )
...
def broadcast_to_user_topic(user_id, topic, message):
    ...
    try:
        ...
        endpoint = os.environ.get("WEBSOCKET_API_ENDPOINT", "").replace(
            "wss://", "https://"
        )
        if not endpoint:
        ...
        print(
            f"Notificação '{topic}' enviada para {sent_count} conexões do usuário {user_id}"
        )
